...
class Composer {
    ...
    protected function getApp() {
        ...
        $config     = new \Microbe\Config\File($vendorPath . '/' . $this->config['group'] . '/' . $this->config['protect'] . '/src/conf');
    ...
    public function __call($name, $arguments) {
        if (!method_exists($this->serviceObject, $name)) {
            return [
                'code' => \Microbe\Service::CODE_SERVICE_NOT_FOUND,
        ...
        try {
            ...
            return [
                'code' => \Microbe\Service::CODE_SUCCESS,
                'msg'  => "Service method [{$this->service}->{$name}] success",
                'data' => $serviceData,
            ];
        } catch (\Exception $exception) {
            return [
                'code' => \Microbe\Service::CODE_SERVICE_EXCEPTION,
