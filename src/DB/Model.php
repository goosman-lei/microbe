...
abstract class Model {
    ...
    public function query($sql, $onReturn = self::RS_ARRAY) {
        ...
        switch ($onReturn) {
            ...
            default:
                ...
                return $rs === FALSE && $handler->errno != self::DUPLICATE_ERRNO ? FALSE : TRUE;
            break;
        }
    }
    ...
    public function getRow($where = array(), $cols = '*', $orderBy = FALSE, $join = FALSE, $groupBy = FALSE, $having = FALSE, $tableOptions = FALSE) {
        $rows = $this->getRows($where, $cols, 1, 0, $orderBy, $join, $groupBy, $having, $tableOptions);
        if (!is_array($rows)) {
            return FALSE;
        }
